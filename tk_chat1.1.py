...
# 左右次框架
frm_left = tk.LabelFrame(frm, background='#EEE')
...
scroll_left = tk.Scrollbar(frm_left, command=frm_left.yview)
scroll_left.pack(side='right',fill=tk.Y)
# 设置左侧框的纵向滚动影响scroll_left滚动条
frm_left.configure(yscrollcommand=scroll_left.set)
...
for i, name in enumerate(list_friends):
	...
	# 创建并放置标签
	btn_friend = tk.Button(frm_left, text=name, fg=fg_color, bg=bg_color, font=('Arial', 20, 'bold'))
